...
export function ProductAnalysisForm({ onClose, onSubmit }: ProductAnalysisFormProps) {
  ...
  const translations = {
    ...
    kz: {
      ...
      "analysis.enter.competitor": "Жоғары рейтингі бар бәсекелестің SKU енгізіңіз",
      "analysis.competitor": "SKU (үздік бәсекелес)",
      "analysis.note": "Тауарыңыз туралы көбірек деректер алу үшін бәсекелестің аналогының SKU енгізу қажет. Қосымша.",
    ...
    en: {
      ...
      "analysis.note": "To get more data about your product, you need to enter a competitor's analog SKU. Optional.",
      ...
      <div className="max-w-md mx-auto pt-10">
        ...
        <div className="flex items-center justify-center mb-8">
          <div className="bg-blue-400 text-white py-3 px-6 rounded-full">
            <h2 className="text-lg font-medium">{translate("analysis.title")}</h2>
          </div>
        </div>

        <form className="space-y-6" onSubmit={handleSubmit(onFormSubmit)}>
          <div className="space-y-2">
            <p className="text-sm text-gray-500">{translate("analysis.enter.sku")}</p>
            <div>
              <Input
                id="sku"
              ...
              />
              {errors.sku && <p className="text-red-500 text-xs mt-1">{translate("analysis.required")}</p>}
            </div>
          </div>

          <div className="space-y-2">
            <p className="text-sm text-gray-500">{translate("analysis.enter.competitor")}</p>
            <div>
              <Input
                id="competitorSku"
              ...
              />
              {errors.competitorSku && <p className="text-red-500 text-xs mt-1">{translate("analysis.required")}</p>}
            </div>
            <p className="text-xs text-gray-500 mt-1">{translate("analysis.note")}</p>
          </div>

          <Button type="submit" className="w-full bg-blue-400 hover:bg-blue-500 text-white rounded-full">

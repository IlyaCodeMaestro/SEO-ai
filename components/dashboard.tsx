...
import { ProductAnalysisDetails } from "@/components/product-analysis-details"
import { ProductAnalysisModal } from "@/components/product-analysis-modal"
import { ProductAnalysisResults } from "@/components/product-analysis-results"
import { ProductDescriptionForm } from "@/components/product-description-form"
import { ProductDescriptionDetails } from "@/components/product-description-details"
import { ProductDescriptionModal } from "@/components/product-description-modal"
import { ProcessingView } from "@/components/processing-view"
import { ArchiveView } from "@/components/archive-view"
import { ArchiveItemDetails } from "@/components/archive-item-details"
...
import { BalanceTopupPanel } from "./balance-topup-panel"
import { BalanceHistoryPanel } from "./balance-history-panel"
import { BonusExchangePanel } from "./bonus-exchange-panel"
import { BonusStatementPanel } from "./bonus-statement-panel"
import { ReferralStatementPanel } from "./referral-statement-panel"
import { TariffPanel } from "./tariff-panel"
import { ActiveDevicesPanel } from "./active-devices-panel"
import { DeleteAccountPanel } from "./delete-account-panel"
import { ShareMenu } from "./share-menu"
import { PartnerStandardPanel } from "./partner-standard-panel"
import { PartnerPremiumPanel } from "./partner-premium-panel"
import { FeedbackFaqPanel } from "./feedback-faq-panel"
import { FeedbackRecommendationsPanel } from "./feedback-recommendations-panel"
import { FeedbackComplaintPanel } from "./feedback-complaint-panel"
import { FeedbackOtherPanel } from "./feedback-other-panel"
...
export function Dashboard() {
  ...
  const [descriptionStep, setDescriptionStep] = useState<DescriptionStep>("form")
  ...
  const handleAnalysisFormSubmit = (data: { sku: string; competitorSku: string }) => {
    setProductData(data)
    setAnalysisStep("details")
  }

  const handleDescriptionFormSubmit = (data: { sku: string; competitorSku: string }) => {
    setProductData(data)
  ...
  const isFeedbackPanel =
    activePanel === "faq" || activePanel === "recommendations" || activePanel === "complaint" || activePanel === "other"

  // Рендерим содержимое левого блока в зависимости от активной вкладки
  const renderLeftContent = () => {
    switch (activeTab) {
      case "main":
        return <MainContent onOpenPanel={handleOpenPanel} onOpenProcessing={handleOpenProcessing} />
      case "archive":
  ...
  const renderRightContent = () => {
    if (activeTab === "main") {
      if (activePanel === "product-analysis") {
        if (analysisStep === "form") {
          return <ProductAnalysisForm onClose={handleClosePanel} onSubmit={handleAnalysisFormSubmit} />
        ...
        if (descriptionStep === "form") {
          return <ProductDescriptionForm onClose={handleClosePanel} onSubmit={handleDescriptionFormSubmit} />
    ...
    } else if (activeTab === "archive" && activePanel === "archive-item" && selectedArchiveItem) {
      return <ArchiveItemDetails onClose={handleClosePanel} item={selectedArchiveItem} />
      ...
      if (activePanel === "balance-topup") {
        ...
        return <BonusStatementPanel onClose />
      } else if (activePanel === "bonus-statement") {
  ...
  if (isMobile) {
    ...
    if (activePanel) {
      return (
        <div className="flex flex-1 justify-center w-full min-h-[calc(100vh-65px)]">
          <div className="w-full bg-white rounded-none">{renderRightContent()}</div>
          {shareMenuOpen && (
            <ShareMenu title={shareContent.title} content={shareContent.content} onClose={handleCloseShareMenu} />
          )}
        </div>
      )
    }
    ...
    return (
      <div className="flex flex-1 justify-center w-full min-h-[calc(100vh-65px)]">
        <div className="w-full bg-[#F6F6F6] rounded-none">{renderLeftContent()}</div>
        {shareMenuOpen && (
          <ShareMenu title={shareContent.title} content={shareContent.content} onClose={handleCloseShareMenu} />
        )}
      </div>
  ...
  return (
    <div className="flex flex-1 justify-center w-full max-w-[1600px] mx-auto pl-0 pr-4 md:pl-0 md:pr-6 lg:pl-0 lg:pr-8 xl:pl-0 xl:pr-12 min-h-[calc(100vh-110px)]">
      {/* Левая панель - всегда видима */}
      <div className="w-[380px] bg-[#F6F6F6] rounded-[25px] mt-[30px] z-10 ml-0 mr-2 md:ml-0 md:mr-3 lg:ml-0 lg:mr-4">
        {renderLeftContent()}
      </div>
      {/* Правая панель - по умолчанию пустая */}
      <div className="flex-1 max-w-[794px] relative mx-2 md:mx-3 lg:mx-4">
        <div className={`w-full bg-white rounded-[25px] mt-[30px] ${isFeedbackPanel ? "max-w-[900px]" : ""}`}>
          {renderRightContent()}
        </div>
      </div>
      {shareMenuOpen && (

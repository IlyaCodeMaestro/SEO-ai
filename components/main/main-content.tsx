...
import { useGetMainInfoQuery, useGetProcessListQuery } from "@/store/services/main"
...
export function MainContent({ onOpenPanel, onOpenProcessing }: MainContentProps) {
  ...
  const { data: processData, isLoading: processLoading } = useGetProcessListQuery()
  const { language, setLanguage, t } = useLanguage()

  // Check if there are any items in processing
  const hasContextItems = processingItems && processingItems.length > 0
  const hasApiItems = processData && processData.card_dates !== null && processData.card_dates.length > 0
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="pt-4 pb-0 px-6">
        <h2 className="text-[#1950DF] font-medium mb-4 text-center text-xl md:text-md">Главная</h2>
        <h1 className="text-[25px] md:text-xl font-normal mb-4">Здравствуйте, Ерлан!</h1>

        {/* First card */}
        <div
          className={getCardStyle("product-analysis", "from-[#64cada] to-[#4169E1]")}
          onClick={() => handleCardClick("product-analysis", () => onOpenPanel("product-analysis"))}
        >
          <h2 className="text-[24px] font-medium mb-3 ml-2">Анализ карточки товара</h2>
          <p className="text-base md:text-sm mb-1 ml-2 font-medium">Проверьте качество SEO и текста</p>
          <p className="text-base md:text-sm mb-5 ml-2 font-thin">
            Доступно {mainData && mainData?.tariff?.analyses} анализов
          </p>
          <div className="ml-2">
            <Button
              variant="secondary"
              ...
              onClick={(e) => {
                ...
                handleCardClick("product-analysis", () => onOpenPanel("product-analysis"))
              }}
            >
              {t("tariff.switch")}
            </Button>
        ...
        {/* Second card */}
        <div
          className={getCardStyle("product-description", "from-[#0d52ff] to-[rgba(11,60,187,1)]")}
          onClick={() => handleCardClick("product-description", () => onOpenPanel("product-description"))}
        >
          <h2 className="text-[22px] font-medium mb-3 ml-2">Описание карточки товара</h2>
          ...
          <p className="text-base mb-5 ml-2 text-[15px] md:text-sm font-thin">
            Доступно {mainData && mainData?.tariff?.descriptions} описаний
          </p>
          <div className="ml-2">
            <Button
              variant="secondary"
              ...
              onClick={(e) => {
                e.stopPropagation() // Prevent triggering parent onClick
                handleCardClick("product-description", () => onOpenPanel("product-description"))
              }}
            >
              {t("tariff.switch")}
            </Button>

...
export function ProcessingView({ onClose }: ProcessingViewProps) {
  const { data: processingItems, isLoading, error } = useGetProcessListQuery()
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        {processingItems?.card_dates?.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            Нет товаров в обработке
          </div>
        ) : (
          <div className="space-y-4">
            {processingItems?.card_dates?.map((item) => {
              item.cards
              return (
                <>
                  {item.cards?.map(card => {
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <div className="flex-1">
                          ...
                          {/* <p className="text-sm text-gray-500">{card.sku}</p> */}
                        </div>
                        <div className="flex items-center">

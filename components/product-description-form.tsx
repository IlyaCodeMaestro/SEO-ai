...
export function ProductDescriptionForm({ onClose, onSubmit }: ProductDescriptionFormProps) {
  ...
  const translations = {
    ru: {
      ...
      "description.no.card": "Если у поставщика нет карточки, введите SKU похожего товара",
      "description.sku": "SKU (описываемый товар)",
      "description.enter.competitor": "Введите SKU конкурента с высоким рейтингом",
    ...
    kz: {
      ...
      "description.no.card": "Егер жеткізушіде карточка болмаса, ұқсас тауардың SKU енгізіңіз",
      "description.sku": "SKU (сипатталатын тауар)",
      "description.enter.competitor": "Жоғары рейтингі бар бәсекелестің SKU енгізіңіз",
    ...
    en: {
      ...
      "description.no.card": "If the supplier doesn't have a card, enter the SKU of a similar product",
      ...
      "description.note": "To get more data about your product, you need to enter a competitor's analog SKU. Optional.",
      ...
      <div className="max-w-md mx-auto pt-10">
        ...
        <div className="flex items-center justify-center mb-8">
          <div className="bg-blue-500 text-white py-3 px-6 rounded-full">
            <h2 className="text-lg font-medium">{translate("description.title")}</h2>
          </div>
        </div>

        <form className="space-y-6" onSubmit={handleSubmit(onFormSubmit)}>
          <div className="space-y-2">
            <p className="text-sm text-gray-500">{translate("description.enter.sku")}</p>
            <p className="text-xs text-gray-500">{translate("description.no.card")}</p>
            <div>
              <Input
                id="sku"
                className={`mt-1 bg-gray-50 dark:bg-gray-700 ${errors.sku ? "border-red-500" : ""}`}
                placeholder="SKU12345"
                {...register("sku", { required: true })}
              />
              {errors.sku && <p className="text-red-500 text-xs mt-1">{translate("description.required")}</p>}
            </div>
          </div>

          <div className="space-y-2">
            <p className="text-sm text-gray-500">{translate("description.enter.competitor")}</p>
            <div>
              <Input
                id="competitorSku"
                className={`mt-1 bg-gray-50 dark:bg-gray-700 ${errors.competitorSku ? "border-red-500" : ""}`}
                placeholder="SKU67890"
                {...register("competitorSku", { required: false })}
              />
              {errors.competitorSku && <p className="text-red-500 text-xs mt-1">{translate("description.required")}</p>}
            </div>
            <p className="text-xs text-gray-500 mt-1">{translate("description.note")}</p>
          </div>
